...
bsig0 :: Word32 -> Word32
bsig0 x = foldl' xor 0 [rotateR x 2,
                        rotateR x 13,
                        rotateR x 22]

-- BSIG1(x) = ROTR^6(x) XOR ROTR^11(x) XOR ROTR^25(x)
bsig1 :: Word32 -> Word32
bsig1 x = foldl' xor 0 [rotateR x 6,
                        rotateR x 11,
                        rotateR x 25]

-- SSIG0(x) = ROTR^7(x) XOR ROTR^18(x) XOR SHR^3(x)
ssig0 :: Word32 -> Word32
ssig0 x = foldl' xor 0 [rotateR x 7,
                        rotateR x 18,
                        shiftR x 3]

-- SSIG1(x) = ROTR^17(x) XOR ROTR^19(x) XOR SHR^10(x)
ssig1 :: Word32 -> Word32
ssig1 x = foldl' xor 0 [rotateR x 17,
                        rotateR x 19,
                        shiftR x 10]
...
{-
 ...
 -
 -}
processW :: Sha256Digest -> Int -> Sha256ArrayW -> Reader Config Sha256Digest
processW digest t w = do
    let a = digest!!0
    let b = digest!!1
    let c = digest!!2
    let d = digest!!3
...
getW :: Sha256ArrayW -> Int -> Sha256ArrayW
getW w t = do
...
processBlock digest block = do
    ...
    let arrW = foldl' getW startW [16..63]

    digestResult <- foldlM (\d t -> processW d t arrW) digest [0..63]
